...
export default function DashboardPage() {
  const summary=coreSummary(), p2=phase2Summary(), payrollRuns=listPayrollRuns();
  ...
  const cards=[
    ...
    {label:"최근 급여",value:latestPayroll?`${v(latestPayroll.payroll_month)} · ${v(latestPayroll.status)}`:"미생성",caption:latestPayroll?`확인필요 ${v(latestPayroll.review_count)}명`:`급여 회차를 생성하세요`,href:"/payroll"},
  ];
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{cards.map(c=><Link key={c.label} href={c.href} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><p className="text-sm font-semibold text-slate-500">{c.label}</p><p className="mt-2 text-2xl font-black text-slate-950">{c.value}</p><p className="mt-2 text-xs text-slate-400">{c.caption}</p></Link>)}</section>
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div><h2 className="font-bold text-slate-950">빠른 업무 시작</h2><p className="mt-1 text-xs text-slate-500">입력한 정보가 배치·근태·연차·급여까지 이어집니다.</p></div><div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <Link href="/payroll" className="rounded-xl border border-slate-200 p-4 hover:border-teal-300"><p className="font-bold">월 급여 생성</p><p className="mt-1 text-xs text-slate-500">근태·급여기준을 연결해 검토·승인·마감</p></Link>
      </div></section>
    ...
    <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5"><p className="font-bold text-blue-950">운영 기준</p><p className="mt-1 text-sm text-blue-800">기존 Excel은 1회 이관·검증 자료일 뿐입니다. 신규 인력·배치·계약·근태·연차·대체근무·급여는 운영 DB가 원본입니다.</p></section>
  </div>;
}
