...
type ShellUser = { display_name:string; role:"owner"|"operator" } | null;
const mainNav: NavItem[] = [
  ...
  { label: "퇴사정산", href: "/exits", icon: "document" },
];
...
export function AppShell({ children, user }: { children: ReactNode; user:ShellUser }) {
  ...
  const roleLabel=user?.role==="owner"?"관리자":"운영 사용자";
  return <div className="min-h-screen bg-slate-50">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-slate-200 bg-white lg:flex">
      ...
      <div className="border-t border-slate-100 p-4"><div className="rounded-xl bg-slate-50 p-3"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-full bg-slate-900 text-xs font-bold text-white">{user?.display_name?.slice(0,1)||"?"}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-900">{user?.display_name||"미인증"}</p><p className="truncate text-[11px] text-slate-500">{roleLabel}</p></div></div><form action={logoutAction} className="mt-3"><button className="w-full rounded-lg border border-slate-200 bg-white py-2 text-xs font-bold text-slate-600 hover:bg-slate-100">로그아웃</button></form></div></div>
    </aside>
    <div className="lg:pl-64">
      <header className="sticky top-0 z-20 flex h-[72px] items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-8">
        ...
        <span className="hidden rounded-full bg-teal-50 px-3 py-1 text-xs font-bold text-teal-800 sm:inline">{roleLabel}</span>
      </header>
